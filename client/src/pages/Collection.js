...
const Collection = () => {
  ...
  return (
    <div className="container" style={{ padding: '2rem 0' }}>
      ...
      <div className="products-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '2rem', marginTop: '2rem' }}>
        <style>{`
        ...
        {products.length === 0 ? (
          ...
          products.map(product => (
            <div key={product.id} className="product-card" style={{ background: 'white', borderRadius: '8px', overflow: 'hidden', boxShadow: 'var(--shadow)' }}>
              ...
              <div style={{ padding: '1rem' }}>
                <h3><Link to={`/product/${product.id}`} style={{ color: 'var(--text-color)', textDecoration: 'none' }}>{product.name}</Link></h3>
                <div style={{ marginBottom: '1rem' }}>
                  {product.discount_price && <span style={{ textDecoration: 'line-through', color: 'var(--text-light)', marginRight: '0.5rem' }}>₹{parseFloat(product.price || 0).toFixed(2)}</span>}
                  <span style={{ fontSize: '1.2rem', fontWeight: '600', color: 'var(--primary-color)' }}>₹{parseFloat(product.discount_price || product.price || 0).toFixed(2)}</span>
                </div>
