...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const result = await query('SELECT COUNT(*) as count FROM cart WHERE user_id = $1', [authResult.user.id]);
    return { statusCode: 200, headers: corsHeaders(), body: JSON.stringify({ count: parseInt(result.rows[0].count) }) };
  } catch (error) {
    console.error('Cart count error:', error);
    return { statusCode: 500, headers: corsHeaders(), body: JSON.stringify({ error: 'Failed to get cart count' }) };
  }
};
